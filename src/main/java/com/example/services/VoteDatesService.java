...
public class VoteDatesService {
    ...
    @Path("/add/{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response createVote(@PathParam("id") Long id, VoteDatesDTO vote) {
    ...
    @DELETE
    @Path("/delete/{id}/{idUser}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response deleteVote(@PathParam("idUser") Long idUser, @PathParam("id") Long id) {
        UserPrio user = entityManager.find(UserPrio.class, idUser);
        RolePrio role = (RolePrio) user.getRole();
        if (user != null) {
            if (role.getCode() == 2) {
                try {
    ...
    @PUT
    @Path("/update/{id}/{idUser}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response updateVote(@PathParam("idUser") Long idUser, @PathParam("id") Long id, VoteDatesDTO vote) {
        UserPrio user = entityManager.find(UserPrio.class, idUser);
        RolePrio role = (RolePrio) user.getRole();
        if (user != null) {
            if (role.getCode() == 2) {
